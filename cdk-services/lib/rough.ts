...
export class StateMachine extends Construct {
  ...
  constructor(scope: Construct, id: string) {
    ...
    // Flights
    let reserveFlightLambda = this.createLambda(
      this,
      "reserveFlightLambdaHandler",
      "flights/reserveFlight.ts",
      flightTable
    );
    let confirmFlightLambda = this.createLambda(
      this,
      "confirmFlightLambdaHandler",
      "flights/confirmFlight.ts",
      flightTable
    );
    let cancelFlightLambda = this.createLambda(
      this,
      "cancelFlightLambdaHandler",
      "flights/cancelFlight.ts",
      flightTable
    );

    // Car Rentals
    let reserveRentalLambda = this.createLambda(
      this,
      "reserveRentalLambdaHandler",
      "rentals/reserveRental.ts",
      rentalTable
    );
    let confirmRentalLambda = this.createLambda(
      this,
      "confirmRentalLambdaHandler",
      "rentals/confirmRental.ts",
      rentalTable
    );
    let cancelRentalLambda = this.createLambda(
      this,
      "cancelRentalLambdaHandler",
      "rentals/cancelRental.ts",
      rentalTable
    );

    // Payment
    let processPaymentLambda = this.createLambda(
      this,
      "processPaymentLambdaHandler",
      "payment/processPayment.ts",
      paymentTable
    );
    let refundPaymentLambda = this.createLambda(
      this,
      "refundPaymentLambdaHandler",
      "payment/refundPayment.ts",
      paymentTable
    );
    ...
    const reservationFailed = new sfn.Fail(this, "Reservation Failed", {
      error: "Job Failed",
    });
    const reservationSucceeded = new sfn.Succeed(
      this,
    ...
    const snsNotificationFailure = new tasks.SnsPublish(
      this,
      "SendingSMSFailure",
      {
        topic: topic,
    ...
    const snsNotificationSuccess = new tasks.SnsPublish(
      this,
      "SendingSMSSuccess",
      {
        topic: topic,
        ...
        message: sfn.TaskInput.fromText(
          "Your Travel Reservation is Successful"
        ),
      }
    );
    ...
    const cancelRentalReservation = new tasks.LambdaInvoke(
      this,
    ...
    const definition = sfn.Chain.start(reserveFlight)
